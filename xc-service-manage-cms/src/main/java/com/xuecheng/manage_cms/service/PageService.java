...
//    }
    ...
    //保存页面静态化
    public ResponseResult staticState(QueryStaticRequest queryStaticRequest){
        //页面静态化文件存储到GridFs中
        CmsPage cmsPage = savaHtml(queryStaticRequest.getPageId(), queryStaticRequest.getHtml());
        if(cmsPage != null) {
            return new ResponseResult(CommonCode.SUCCESS);
        }
        return new ResponseResult(CommonCode.FAIL);
    ...
    public ResponseResult postTest(String pageId){
        //向MQ发消息
        sendPostPage(pageId);
        return new ResponseResult(CommonCode.SUCCESS);
    }
...
//    public CmsSiteResult findAllCmsSite(){
//        List<CmsSite> cmsSiteList = cmsSiteRepository.findAll();
//        return new CmsSiteResult(CommonCode.SUCCESS, cmsSiteList);
//    }
